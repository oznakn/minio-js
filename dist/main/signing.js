...
function signV4(request, accessKey, secretKey, region, requestDate, signingHost) {
  ...
  var canonicalRequest = getCanonicalRequest(request.method, request.path, request.headers, signedHeaders, sha256sum, signingHost);
...
function presignSignatureV4(request, accessKey, secretKey, sessionToken, region, requestDate, expires, signingHost) {
  if (!(0, _helpers.isObject)(request)) {
    throw new TypeError('request should be of type "object"');
  }
  ...
  var canonicalRequest = getCanonicalRequest(request.method, path, request.headers, signedHeaders, hashedPayload, signingHost);
  var stringToSign = getStringToSign(canonicalRequest, requestDate, region);
  var signingKey = getSigningKey(requestDate, region, secretKey);
